...
- **Catalog checksums for the six curated models are placeholders** pending a `catgen` CI run against
  the real multi-GB GGUFs. Downloading such an entry is refused up front (rather than wasting
  bandwidth) until a real SHA-256 is pinned. The checksum *gate itself* is fully exercised.
